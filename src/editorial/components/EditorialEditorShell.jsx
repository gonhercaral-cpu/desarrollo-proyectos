...
function EditorialEditorReady({ project, documents, profile, theme, onToggleTheme, onBack }) {
  ...
  const variableValues = useMemo(() => buildEditorialVariableValues({ project, document: navigation.document, page: activePage, section: activeSection, sections: navigation.sections, numbering: navigation.numbering, customVariables: design.variables }), [activePage, activeSection, design.variables, navigation.document, navigation.numbering, navigation.sections, project]);
  const resolvedElements = useMemo(() => resolveLocalElements(editor.elements, { stylesById: design.stylesById, componentsById: design.componentsById, variables: variableValues }), [design.componentsById, design.stylesById, editor.elements, variableValues]);
  ...
  const activeMasterElements = useMemo(() => activePageMaster ? resolveMasterElements(activePageMaster.elements || [], activePage?.masterOverrides, { stylesById: design.stylesById, variables: variableValues }) : [], [activePage?.masterOverrides, activePageMaster, design.stylesById, variableValues]);
  ...
  async function submitDesignDialog(values) {
    ...
    try {
      ...
      else if (action === "create-component") await createEditorialComponent({ projectId: project.id, values, elements: resolvedSelectedElements, user: profile });
  ...
  const spreadSlots = useMemo(() => {
    if (editorMode.kind !== "page") {
      ...
      return [{ page: surface, active: true, metrics: getPageMetrics(getPageProject(project, surface)), background: surface.background, elements: resolvedElements, backgroundElements: [], selectedElement: resolvedSelectedElement, selectedIds: editor.selectedIds, numberLabel: editorMode.kind === "master" ? "Maestra" : "Componente" }];
    }
    ...
    return slotPages.map((page) => {
      ...
      const variables = active ? variableValues : buildEditorialVariableValues({ project, document: navigation.document, page, section, sections: navigation.sections, numbering: navigation.numbering, customVariables: design.variables });
      const localElements = active ? resolvedElements : resolveLocalElements(page?.id === secondaryPage?.id ? secondaryElements : [], { stylesById: design.stylesById, componentsById: design.componentsById, variables });
      ...
      return {
        ...
        backgroundElements: active ? activeMasterElements : master ? resolveMasterElements(master.elements || [], page.masterOverrides, { stylesById: design.stylesById, variables }) : [],
  ...
  }, [activeComponent, activeMaster, activeMasterElements, activePage, design.componentsById, design.mastersById, design.stylesById, design.variables, editor.selectedIds, editorMode.id, editorMode.kind, navigation.document, navigation.numbering, navigation.sections, project, resolvedElements, resolvedSelectedElement, secondaryElements, secondaryPage?.id, spread.left, spread.right, spread.standalone, variableValues, viewMode]);
  ...
  return (
    <div className={`editorial-editor-shell ${leftOpen ? "left-open" : "left-closed"} ${rightOpen ? "right-open" : "right-closed"} ${bottomOpen ? "bottom-open" : "bottom-closed"} context-${editorMode.kind}`}>
      <header className="editorial-editor-topbar">
        ...
        <div className="editorial-editor-top-actions"><span className={`editorial-save-status ${editor.saveStatus}`} title={editor.saveError || statusLabels[editor.saveStatus]}><i />{statusLabels[editor.saveStatus]}</span><button type="button" className="editorial-top-action-button" onClick={() => setQuickPreviewOpen(true)}><EditorialIcon name="eye" size={17} /> Vista rápida</button><button type="button" className="editorial-top-icon-button" onClick={onToggleTheme} aria-label={theme === "dark" ? "Usar modo claro" : "Usar modo oscuro"}><EditorialIcon name={theme === "dark" ? "sun" : "moon"} /></button><span className="editorial-user-avatar">{getInitials(profile)}</span></div>
      </header>
      ...
      <nav className="editorial-editor-rail" aria-label="Navegación editorial">{RAIL_ITEMS.map(([name, label]) => <button type="button" className={activeRail === name ? "active" : ""} onClick={() => setActiveRail(name)} key={name}><EditorialIcon name={name} /><span>{label}</span></button>)}<button type="button" className="editorial-rail-back" onClick={handleBack}><EditorialIcon name="arrowLeft" /><span>Proyectos</span></button></nav>
      {leftOpen && <EditorialStructurePanel projectId={project.id} activeRail={activeRail} railItems={RAIL_ITEMS} navigation={navigation} ordering={ordering} activeElements={editorMode.kind === "page" ? editor.elements : []} onSelectPage={handleSelectPage} onCreatePage={openCreatePage} onCreateSection={(initialType) => setSectionDialog({ initialType })} onEditSection={(section) => setSectionDialog({ section })} onDeleteSection={(section) => setDeleteTarget({ kind: "section", item: section })} onPageAction={handlePageAction} design={design} editor={displayEditor} editorMode={editorMode} onDesignAction={handleDesignAction} canManageInstitutional={String(profile?.role || "").toLowerCase() === "admin"} />}
      <EditorialWorkspace ref={workspaceRef} metrics={metrics} zoom={zoom} viewMode={editorMode.kind === "page" ? viewMode : "single"} showRulers={showRulers} guideSettings={guideSettings} spreadSlots={spreadSlots} onZoomChange={setZoom} onSelectPage={handleSelectPage} onSelectElement={editor.select} onChangeElement={handleCanvasElementChange} />
      {rightOpen && <EditorialInspectorPanel activeTab={activeInspector} onChangeTab={setActiveInspector} editor={editor} displayEditor={displayEditor} design={design} page={editorMode.kind === "page" ? activePage : null} master={activePageMaster} onDesignAction={handleDesignAction} />}
      {bottomOpen && <EditorialBottomPanel project={project} navigation={navigation} ordering={ordering} activeElements={editorMode.kind === "page" ? editor.elements : []} onSelectPage={handleSelectPage} />}
